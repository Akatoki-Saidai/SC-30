...
#     (camera.py の capture_and_detect / motordrive.py の 5引数 move など)
# ============================================================
...
# ------------------------------------------------------------
# カメラ
#   camera.py (V3) の Camera を使う。
#   API: capture_and_detect(is_inverted) -> (frame, x_percent, order, red_area)
# ------------------------------------------------------------
CAMERA_MODEL_PATH = "./my_custom_model.pt"
CAMERA_DEBUG = True
...
def setup_devices():
    ...
    try:
        if Camera is not None:
            ctx.cam = Camera(model_path=CAMERA_MODEL_PATH, debug=CAMERA_DEBUG)
            say("Camera 初期化完了")
        else:
...
def run_phase4(ctx):
    ...
    while True:
        try:
            ...
            # --- カメラで画像取得 & 推論 ---
            # camera.py の API: (frame, x_percent, order, red_area)
            frame, x_percent, order, red_area = ctx.cam.capture_and_detect(
                is_inverted=inverted
            )

            log_msg("camera_order", order)
            log_msg("camera_area", red_area)
            log_msg("goal_relative_x", x_percent)

            if SHOW_CAMERA_WINDOW and cv2 is not None and frame is not None:
                try:
